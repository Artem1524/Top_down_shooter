...
namespace TDShooter
{
    public static class DebugUtility
    {
        ...
        public static void HandleErrorIfNullGetComponentInChildren<TO, TS>(Component component, Component source,
            GameObject onObject)
        {
#if UNITY_EDITOR
            if (component == null)
            {
                Debug.LogError("Error: Component of type " + typeof(TS) + " on GameObject " + source.gameObject.name +
                               " expected to find a component of type " + typeof(TO) + " in children of GameObject " +
                               onObject.name + ", but none were found.");
            }
#endif
        }

        public static void DebugMessage(Component component, string message)
        {
#if UNITY_EDITOR
            Debug.Log(component.name + " " + message);
#endif
        }
